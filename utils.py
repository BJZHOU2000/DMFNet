...
class TrainSetLoader(Dataset):
    """
       TrainSetLoader继承Dataset, 重载了__init__(), __getitem__(), __len__()
    ...
    def _sync_transform(self, img, mask):
        ...
        if random.random() < 0.5:
            img = img.transpose(Image.FLIP_LEFT_RIGHT) #FLIP_LEFT_RIGHT--左右翻转；FLIP_TOP_BOTTOM--上下翻转
            mask = mask.transpose(Image.FLIP_LEFT_RIGHT)
        crop_size = self.crop_size
        # random scale (short edge)
            #random.randint(a, b) 是 Python 中 random 模块中的函数，它可以生成并返回一个在给定区间[a,b]（包括a和b）内的随机整数。
        ...
        if h > w:
            ...
            short_size = oh
        img = img.resize((ow, oh), Image.BILINEAR) #Image.BILINEAR：双线性
        mask = mask.resize((ow, oh), Image.NEAREST) #Image.NEAREST：低质量
        # pad crop
        ...
        # final transform
        img,img_1, mask = np.array(img), np.array(img_1), np.array(mask, dtype=np.float32)
        return img,img_1, mask
    ...
    def __getitem__(self, idx):
        ...
        # synchronized transform
        img, img_1, mask = self._sync_transform(img, mask)
        ...
        return img, img_1, torch.from_numpy(mask) #img_id[-1]
...
def save_model_and_result(dt_string, epoch,train_loss, test_loss, best_iou, recall, precision, save_mIoU_dir, save_other_metric_dir):
    ...
    with open(save_other_metric_dir, 'a') as f:
        ...
        f.write('\n')
# 保存训练的数据
def save_model(mean_IOU, save_dir, save_prefix, train_loss, test_loss, recall, precision, epoch):
    save_mIoU_dir = 'result/' + save_dir + '/' + save_prefix + '_IoU_IoU.log'
    save_other_metric_dir = 'result/' + save_dir + '/' + save_prefix + '_IoU_other_metric.log'
    now = datetime.now()
    dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
    best_iou = mean_IOU
...
def save_ROC_for_test(dataset_dir, st_model, epochs, best_iou, ture_positive_rate, false_positive_rate ):
    now = datetime.now()
    dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
    with open(dataset_dir + '/' +'value_result'+'/'+ st_model + '_best_roc.log', 'a') as f:
        f.write(dt_string)
        f.write('-')
        f.write(str(epochs))
        f.write('\n')
        f.write('ture_positive_rate-----:')
        for i in range(len(ture_positive_rate)):
            f.write('   ')
            f.write(str(round(ture_positive_rate[i], 8)))
            f.write('   ')
        f.write('\n')
